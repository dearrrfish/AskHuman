...
| P6 | 回位 | 原输入位置能容纳按固定态高度投影出的可见部分后自动回位；高输入框随后随滚动逐步完整露出 |
...
| P18 | 编辑器几何 | 纵向题卡的折叠态与聚焦空态都和单行预设答案等高；未聚焦 hover 沿用预设答案底色。空白时工具按钮同行靠右，出现第一个字符后文字恢复整行宽度、按钮移到输入框内部下一行；后续多行只增长文字区。自增高不得在 live textarea 上临时写 `height:auto`；增长只写最终高度，缩短用不参与正文流的镜像测量。focus ring 由 `.input-wrap` 的非滚动绘制层承载，不直接绑在会改变高度的可滚动 textarea 阴影上。已展开 textarea 在 blur 时保留现有测量高度，只有确实折叠时才清除内联高度。 |
...
| 向上回看，owner 原位按固定态高度投影出的底边越过内容区底边 | 不变 | `owner` | 同一编辑器移动到 footer 上方 |
| 原位能连续承接固定态可见高度 | 不变 | `null` | 同一编辑器自动回到题内，超出部分随滚动逐步露出 |
...
6. 把 textarea 按固定态上限投影后，home marker 的投影底边即将越过内容区底边，表示用户正在
   回看它上方的内容。

短输入框的投影高度等于内联高度，因此仍在底边开始被裁切时固定。高于固定态上限的输入框先被
`.content` 底边逐步裁切，剩余可见高度接近固定态高度时才移动；切换前后的顶部与可见高度因此连续，
不会因 `240px → 120px` 的高度收缩突然拉开和预设答案的距离。

### 5.3 回位条件与滞回

固定后仍使用同一条固定态高度投影边界：把撤掉固定区后 `.content` 将释放的高度计入可用视口；投影
底边进入该释放后的视口，并保留约 8–12px 安全间距时即回位。高输入框回位时只要求原位能承接固定态
可见部分，其余内联高度随后随向下滚动逐步露出；不会从 `120px` 瞬间向上展开到 `240px`。附件仍不
参与这条 home marker 判定。固定阈值与回位阈值使用小幅滞回，避免边界附近因布局取整、textarea
自增高或 dock 出现导致反复固定 / 回位。
...
- `ResizeObserver` 只在内联态记录编辑器高度；固定时原 anchor 保留该高度，避免正文 `scrollHeight`
  塌陷、Message 位置跳动。
- textarea 自增高读取 live `scrollHeight` 后只在确需增长时写一次最终高度；普通字符不写高度。删除、
  undo、替换等可能缩短内容时，在固定定位、不可见且不参与正文流的镜像 textarea 上测量，再对 live
  textarea 一次性写入结果。不得用 `height:auto → scrollHeight → height` 的 live 两阶段测量，以免
  WebKit 在连续换行输入时反复调整 `.content.scrollTop`。
...
1. 单题长 Message：输入框激活后向上滚，输入框在内容区底边连续停靠；在 Message 选字后仍保留，
   可继续输入。
2. 高输入框向上回看时逐步裁切到固定态高度后连续停靠；向下回看时在原位能承接固定态高度后连续
   回位并逐步完整露出。两个方向都不因 `240px ↔ 120px` 产生顶部跳变，边界附近不闪烁、不振荡。
3. 输入框从视口顶部离开时不固定。
